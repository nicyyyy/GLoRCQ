...
def get_c4_(nsamples, seed, seqlen, model):
    ...
    traindata = load_dataset(
        'allenai/c4', 'allenai--c4', data_files={'train': 'en/c4-train.00000-of-01024.json.gz'}, split='train', download_mode="force_redownload",trust_remote_code=True
    )
    valdata = load_dataset(
        'allenai/c4', 'allenai--c4', data_files={'validation': 'en/c4-validation.00000-of-00008.json.gz'}, split='validation', download_mode="force_redownload",trust_remote_code=True
    )
...
def get_c4_2(nsamples, seed, seqlen, model):
    from datasets import load_dataset
...
def get_calibration(data="c4", tokenizer=None, n_samples=512, block_size=512):
    if data == "pileval":
...
def get_test_tokens(name, seed=0, seqlen=2048, model=''):
    train_samples = 0
    if name == 'wikitext2':
        return get_wikitext2_(train_samples, seed, seqlen,
                             model)[1]['input_ids']
    else:
        raise Exception