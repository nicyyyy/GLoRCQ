...
class GraphCompatibleMoeBlock(nn.Module):
    ...
    def __init__(self, original_block):
        ...
        # --- Inference-mechanism ABLATION switches (paper S5.3 speed ablation) ---
        # All default OFF => byte-identical to HEAD for every model. Each flag
        # DISABLES one co-design mechanism so its contribution can be measured:
        #   GLORCQ_ABL_NO_OVERLAP=1 : STANDARD (eager) decode only — run the
        #       correction serially on the main stream instead of overlapping it
        #       on the side stream. Same math/order, just no concurrency.
        #   GLORCQ_ABL_LORA_LOOP=1  : GRAPH-path forward only — issue the
        #       corrections as a per-expert loop of small GEMVs (2 per expert
        #       per proj) instead of one wide U GEMM + one grouped SV kernel.
        #       Values identical (same slices of the same Sa-fused tensors).
        # (A "no pooled-basis reuse" speed flag is deliberately absent: with
        # per-expert activation scales Sa, x@U cannot be shared across experts
        # mathematically — the pool's dividend is storage, measured in S5.3.)
        self._abl_no_overlap = _os.environ.get("GLORCQ_ABL_NO_OVERLAP") == "1"
        self._abl_lora_loop = _os.environ.get("GLORCQ_ABL_LORA_LOOP") == "1"
        #   GLORCQ_ABL_GRAPH_OVERLAP=1 : GRAPH-path forward — compute the
        #       batched corrections on a side stream, overlapped with the
        #       backbone kernels, fork/join captured as graph dependency edges
        #       (so, unlike the eager path, no per-token launch cost). Same
        #       ops and add order => bit-identical outputs. Do not combine
        #       with GLORCQ_ABL_LORA_LOOP.
        self._abl_graph_overlap = _os.environ.get("GLORCQ_ABL_GRAPH_OVERLAP") == "1"
    ...
    def _batched_proj_forward_vq4(self, x, expert_indices, experts_data,
        ...
        lora_outs = [None] * K_exp
        lora_cat = None
        if self._inline_lora or self._abl_no_overlap:
            side = None
    ...
    def _forward_graph_vq4(self, hidden_states, routing_weights,
        ...
        # ABLATION (GLORCQ_ABL_GRAPH_OVERLAP=1): compute the batched gate/up
        # corrections on a side stream, overlapped with the gate/up backbone
        # kernels below. The fork/join is captured into the CUDA graph as
        # dependency edges, so — unlike the eager path — it costs no per-token
        # launch overhead. Same ops, same add order => bit-identical outputs.
        _ovl = self._abl_graph_overlap and N == 1 and not self._abl_lora_loop
        _ovl_lora_gate = _ovl_lora_up = None
        if _ovl:
            _side = self._get_side_stream()
            _side.wait_stream(torch.cuda.current_stream())     # x_h ready
            with torch.cuda.stream(_side):
                _rg, _ru = self._vq4_gate_rank, self._vq4_up_rank
                if getattr(self, '_vq4_fused_gate_up_U', False):
                    _a_cat = x_h @ self._vq4_U_sa_flat_gate_up
                    _a_g = _a_cat[:, :E * _rg]
                    _a_u = _a_cat[:, E * _rg:]
                else:
                    _a_g = x_h @ self._vq4_U_sa_flat_gate
                    _a_u = x_h @ self._vq4_U_sa_flat_up
                _ovl_lora_gate = lora_grouped_gemv(
                    _a_g.view(E, _rg), self._vq4_SV_T_gate).unsqueeze(0)
                _ovl_lora_up = lora_grouped_gemv(
                    _a_u.view(E, _ru), self._vq4_SV_T_up).unsqueeze(0)
        ...
        if _ovl:
            # Overlap ablation: corrections were launched on the side stream
            # before the backbone; join here, right before the add.
            torch.cuda.current_stream().wait_stream(self._get_side_stream())
            lora_gate = _ovl_lora_gate
            a_flat = a_flat_u = None
        elif self._abl_lora_loop and N == 1:
            # ABLATION (GLORCQ_ABL_LORA_LOOP=1): per-expert correction loop —
            # 2 small GEMVs per expert instead of one wide GEMM + one grouped
            # kernel. Same Sa-fused tensors, same values, E times the launches.
            a_flat = a_flat_u = None
            lora_gate = torch.cat(
                [x_h @ self._vq4_U_sa_gate[e] @ self._vq4_SV_T_gate[e]
                 for e in range(E)], dim=0).unsqueeze(0)               # (1, E, out_d)
        else:
        ...
        if _ovl:
            lora_up = _ovl_lora_up   # already joined at the gate add
        elif self._abl_lora_loop and N == 1:
            # ABLATION: per-expert loop (see gate_proj above).
            lora_up = torch.cat(
                [x_h @ self._vq4_U_sa_up[e] @ self._vq4_SV_T_up[e]
                 for e in range(E)], dim=0).unsqueeze(0)
        else:
        ...
        # Overlap ablation: down correction on the side stream, overlapped
        # with the down rotation + backbone kernel below.
        _ovl_lora_down = None
        if _ovl:
            _side.wait_stream(torch.cuda.current_stream())        # h_all ready
            with torch.cuda.stream(_side):
                _a_d = lora_u_grouped_gemv(
                    h_all.squeeze(0), self._vq4_U_sa_T_down)      # (E, rank)
                _ovl_lora_down = lora_grouped_gemv(
                    _a_d, self._vq4_SV_T_down).unsqueeze(0)
        ...
        if _ovl:
            torch.cuda.current_stream().wait_stream(self._get_side_stream())
            lora_down = _ovl_lora_down
        elif self._abl_lora_loop and N == 1:
            # ABLATION: per-expert loop — each expert's own input row.
            lora_down = torch.cat(
                [h_all[0, e:e + 1] @ self._vq4_U_sa_down[e] @ self._vq4_SV_T_down[e]
                 for e in range(E)], dim=0).unsqueeze(0)
        elif N == 1:
