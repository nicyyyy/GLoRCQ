...
class GLoRCQGraphWrapper:
    ...
    def capture_graph(self):
        ...
        # ABLATION (paper S5.3): GLORCQ_ABL_GRAPH_NOCAPTURE=1 runs the SAME
        # graph-path forward (_forward_graph_vq4: batched all-expert
        # formulation) but with eager kernel launches instead of a captured
        # CUDA graph. Isolates the contribution of graph capture from the
        # batched formulation itself. Default OFF => byte-identical behavior.
        if os.environ.get("GLORCQ_ABL_GRAPH_NOCAPTURE") == "1":
            if not getattr(self, "_abl_nocapture_announced", False):
                print("[GLoRCQ Graph] ABLATION: capture SKIPPED "
                      "(GLORCQ_ABL_GRAPH_NOCAPTURE=1) -> graph-path forward "
                      "with eager launches.")
                self._abl_nocapture_announced = True
            self.graph = None
            self._abl_force_graphmode = True
            return
    ...
    def generate(self, input_ids, max_new_tokens=100):
        ...
        # all-experts path. Exception: the GLORCQ_ABL_GRAPH_NOCAPTURE ablation
        # deliberately forces the graph-path forward with eager launches.
        self._set_moe_graph_mode(
            self.graph is not None
            or getattr(self, "_abl_force_graphmode", False))
...
def run_speed_benchmark(model, tokenizer, max_batch_size=1, max_seq_len=2048,
    ...
    _abl_nocap = getattr(wrapper, "_abl_force_graphmode", False)
    if _abl_nocap:
        _glabel = (" [ABLATION: graph-path forward, EAGER launches "
                   "(GLORCQ_ABL_GRAPH_NOCAPTURE=1), not a captured graph]")
    elif graph_disabled:
