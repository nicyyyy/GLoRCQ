...
# ============================================================
#
# Runs the full paper evaluation chain for Mixtral-8x7B-v0.1:
#   Step 1: WikiText-2 PPL                   (~20 min, fake-quant model)
#   Step 2: Zero-shot 5 tasks (0-shot)       (~60 min, fake-quant model)
#   Step 3: MMLU (5-shot)                    (~90 min, fake-quant model)
#   Step 4: GLoRCQ inference speed           (~5 min,  real-quant model)
#   Step 5: vLLM FP16 baseline speed         (~10 min, Docker required)
#
# GPU memory requirements:
#   - Steps 1-3 (fake-quant): Mixtral 2-bit ≈ 22 GB  → single 24 GB GPU OK
#   - Step 4 (real-quant):    same as above
#   - Step 5 (vLLM FP16):    Mixtral FP16 ≈ 94 GB   → needs H200/B200 or 2×A100
#     NOTE: If vLLM FP16 OOMs on your GPU, pass the --tp flag (tensor parallel):
#       Modify bench_vllm.sh to add --tensor-parallel-size 2 for 2-GPU setup
#
# Usage:
#   bash exp/paper_eval_mixtral.sh <fake_quant_dir> <real_quant_dir> [gpu_id]
#
# Arguments:
...
#       /data/models/Mixtral-8x7B-v0.1-GLoRCQ-2bit \
#       /data/models/Mixtral-8x7B-v0.1-GLoRCQ-2bit-realquant \
#       0
...
# NOTE: Mixtral FP16 is ~94 GB. This step requires H200 141G / B200 192G,
# or 2×A100 80G with tensor parallelism. Skip on single A100.
echo ""
