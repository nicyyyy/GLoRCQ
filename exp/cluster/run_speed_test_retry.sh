#!/bin/bash
# Real-quant decode-speed benchmark retry: mixtral + qwen3.
# Adds classic PYTORCH_CUDA_ALLOC_CONF (belt+suspenders w/ PYTORCH_ALLOC_CONF)
# to reclaim reserved-but-unallocated fragmentation, and shrinks the graph
# StaticCache (--max_seq_len 384; we only need prompt128+gen128=256) to cut
# several GB of KV cache. GPU4, tmux test:0.
set -uo pipefail
